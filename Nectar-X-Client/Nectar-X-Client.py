...
from PyQt6.QtWidgets import (
    ...
    QCompleter, QSystemTrayIcon, QMenu, QToolButton, QMessageBox
)
    ...
    def run(self):
        try:
            ...
            QMessageBox.critical(
                self,                      # parent widget (e.g. your main window)
                "Worker error",            # title
                f"Worker error: {e}"       # message text
            )
            self._emit_finished()
...
class Assets(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_mid = create_quick_button(find_icon('background/syrup.png'), self.Expand_child)
        ...
        self.btn_mid.setToolTip('Expand')
            ...
            def createWindow(self, window_type):
                ...
                def handle_url_change(url):
                    ...
                    if any(ad_domain in url_str for ad_domain in adblock_list):
                        Notify(f"[AdBlock] Blocked: {url_str}", parent=self)
                        popup_view.close()
                        return
    ...
    def cli_search(self, query):
        ...
        query = query.strip()
        if not query:
            Notify("❌ Empty query!", parent=self)
            return
        ...
        Notify(f"🌐 Opening browser for query: {query}", parent=self)
