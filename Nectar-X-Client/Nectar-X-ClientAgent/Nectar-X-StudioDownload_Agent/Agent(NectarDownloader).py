...
GITHUB_GPU_EXE_URL = 'https://huggingface.co/Zashiron/Nectar-X-Studio-GPU/resolve/main/Nectar-X-Studio(GPU).exe?download=true'
GITHUB_CPU_EXE_URL = 'https://huggingface.co/Zashiron/Nectar-X-Studio-CPU/resolve/main/Nectar-X-Studio(CPU).exe?download=true'
...
class DownloadWorker(QThread):
    ...
    def __init__(self, download_url: str, exe_path: str):
        super().__init__()
        self._stop = False
    ...
    def run(self):
        try:
            ...
            if os.path.exists(self.exe_path):
                downloaded = os.path.getsize(self.exe_path)
                headers = {'Range': f'bytes={downloaded}-'}
                mode = 'ab'
                self.status.emit(f'Resuming download from {downloaded} bytes...')

            response = requests.get(self.download_url, stream=True, headers=headers, timeout=15)
            response.raise_for_status()

            total_size = int(response.headers.get('content-length', 0)) + downloaded
            self.status.emit('Downloading executable...')

            with open(self.exe_path, mode) as f:
            ...
            self.finished.emit(self.exe_path)
...
class DownloaderApp(QWidget):
    def __init__(self):
        ...
        """)
        self.version_combo.addItem('GPU', GITHUB_GPU_EXE_URL)
        self.version_combo.addItem('CPU', GITHUB_CPU_EXE_URL)
    ...
    def start_download(self):
        ...
        # Pick URL based on selection
        download_url = self.version_combo.currentData()
        current_text = self.version_combo.currentText().lower()
        if 'cpu' in current_text:
            exe_path = os.path.join(HOME_DIR, 'Nectar-X-Studio(CPU).exe')
        else:
            exe_path = os.path.join(HOME_DIR, 'Nectar-X-Studio(GPU).exe')

        self.worker = DownloadWorker(download_url, exe_path)
