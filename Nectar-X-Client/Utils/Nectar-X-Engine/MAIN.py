...
GITHUB_ZIP_URL = 'https://huggingface.co/Zashiron/Engine-Models/resolve/main/Engine-Models.zip?download=true'
HOME_DIR = os.path.expanduser('~')
ZIP_PATH = os.path.join(HOME_DIR, 'payload.zip')
EXTRACT_TO = HOME_DIR
...
class DownloadWorker(QThread):
    ...
    def run(self):
        try:
            ...
            response = requests.get(GITHUB_ZIP_URL, stream=True, headers=headers, timeout=15)
            response.raise_for_status()

            total_size = int(response.headers.get('content-length', 0)) + downloaded
            self.status.emit('Downloading ZIP...')
...
class DownloaderApp(QWidget):
    def __init__(self):
        ...
        self.title = QLabel('Nectar-X-Engine 🔻')
    ...
    def start_download(self):
        ...
        self.setWindowTitle('Nectar-X-Model Downloader')
    ...
    def on_finished(self, path: str):
        ...
        QMessageBox.information(self, 'Done', f'Files extracted to:\n{path}')
        self.button_start.setEnabled(True)
