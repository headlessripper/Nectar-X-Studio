...
from scripts.Engine.Logic_Unit.Unit_1.main import query_generator
from scripts.Engine.Logic_Unit.Unit_2.main import model_decides_web_rag
...
class LLM_Engine(QWidget):
    ...
    def init_ui(self):
        ...
        self.rag_web_button.clicked.connect(self.load_web_document_ui)
    ...
    def create_settings_page(self):
        ...
        saved_template = QSettings('Zashiron', 'Nectar-X-Studio').value('chat_template', 'llama-3')  # llama default
    ...
    def load_web_document_ui(self):
        ...
        query, ok = QInputDialog.getText(
            self,
            "Add Web Documents to RAG",
            "Enter a search query to fetch relevant web pages:\n\n"
            "Example: \"LLM vector databases for retrieval\"",
        )
        ...
        if not query:
            QMessageBox.information(
                self,
                "No Search Query",
                "Please enter a search query to fetch web documents.",
            )
            return
        ...
        if not hasattr(self, "rag_engine") or self.rag_engine is None:
            QMessageBox.warning(
                ...
                "Load a model first to start the RAG engine, then try again.",
            )
            return
        ...
        try:
            web_rag.add_web_docs(query, num_results=num, recent="d7")
        except Exception as e:
            QMessageBox.critical(
                self,
                "Web Ingestion Failed",
                f"Could not add web documents for this query.\n\n"
                f"Error: {e}",
            )
            return
        ...
        QMessageBox.information(
            self,
            "Web Documents Added",
            f"Successfully added web documents for:\n\n"
            f"“{query}”\n\n"
            f"Top {num} search results have been indexed by the RAG Engine's store.",
        )
...
#
    ...
    def handle_client(self, client_socket):
        ...
        try:
            ...
            if use_web:
                search_query = query_generator(self.llm_instance, query)
